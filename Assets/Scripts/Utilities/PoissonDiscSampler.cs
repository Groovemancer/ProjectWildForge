...
{
    private const int k = 60; // Maximum number of attempts before marking a sample as inactive.
    ...
    /// <param name="height">each sample's y coordinate will be between [0, height]</param>
    ...
    public IEnumerable<Vector2> Samples()
    {
        ...
        yield return AddSample(new Vector2(RandomUtils.value * rect.width, RandomUtils.value * rect.height));

        while (activeSamples.Count > 0)
        {
            // Pick a random active sample
            int i = (int)RandomUtils.value * activeSamples.Count;
            ...
            for (int j = 0; j < k; ++j)
            {
                float angle = 2 * Mathf.PI * RandomUtils.value;
                float r = Mathf.Sqrt(RandomUtils.value * 3 * radius2 + radius2); // See: http://stackoverflow.com/questions/9048095/create-random-number-within-an-annulus/9048443#9048443
