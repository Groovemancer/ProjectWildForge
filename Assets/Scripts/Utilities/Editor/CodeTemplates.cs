...
public class CodeTemplates
{
    ...
    internal static UnityEngine.Object CreateXml(string pathName, string templatePath)
    {
        string newFilePath = Path.GetFullPath(pathName);
        string fileNameWithoutExtension = Path.GetFileNameWithoutExtension(pathName);
        string className = NormalizeClassName(fileNameWithoutExtension);
    ...
    /// <param name="iconName">The name of the icon image name to use</param>
    public static void CreateFromTemplate(string initialName, string templatePath, string iconName)
    {
        Texture2D scriptIcon = (EditorGUIUtility.IconContent(iconName).image as Texture2D);
