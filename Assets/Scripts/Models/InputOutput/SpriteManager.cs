...
public static class SpriteManager
{
    ...
    public static void Initialize()
    {
        ...
        DebugUtils.LogChannel("SpriteManager", "SpriteManager isInitialized?: " + (isInitialized));
    }
        ...
        if (imageTexture.LoadImage(imageBytes))
        {
            ...
            if (File.Exists(jsonPath))
            {
                ...
                foreach (JObject obj in array)
                {
                    ...
                    {
                        DebugUtils.LogWarningChannel("SpriteManager", obj);
    ...
    {
        string name = ReadJson(string.Empty, obj["name"]);
        int x = ReadJson(0, obj["x"]);
        int y = ReadJson(0, obj["y"]);
        int w = ReadJson(1, obj["w"]);
        int h = ReadJson(1, obj["h"]);

        float pivotX = ReadJson(0.5f, obj["pivotX"]);
        float pivotY = ReadJson(0.5f, obj["pivotY"]);
        if (pivotX < 0 || pivotX > 1 || pivotY < 0 || pivotY > 1)
        {
            DebugUtils.LogWarningChannel("SpriteManager", "Pivot for object " + name + " has pivots of " + pivotX + "," + pivotY);
        }
