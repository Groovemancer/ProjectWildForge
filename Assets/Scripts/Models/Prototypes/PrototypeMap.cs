...
public class PrototypeMap<T> where T : IPrototypable, new()
{
    ...
    public void Add(T proto)
    {
        ...
        {
            DebugUtils.LogWarningChannel("PrototypeMap", string.Format("Trying to register a prototype of type '{0}' which already exists. Overwriting. Old value: {1}", proto.Type, oldProto));
        }
    ...
    public void LoadXmlPrototypes(XmlNode rootNode)
    {
        foreach (XmlNode protoNode in rootNode)
        {
            T prototype = new T();

            prototype.ReadXmlPrototype(protoNode);
