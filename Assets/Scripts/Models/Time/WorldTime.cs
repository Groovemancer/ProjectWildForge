...
public struct WorldTime :IFormattable
{
    ...
    private const int StartingYear = 1065;

    private const int DaysPerSeason = 15;

    private const float RealMinutesToDays = 20; // N number of real minutes per Game Day
    private const float AutsPerRealSecond = 100;

    private const float AutsPerMinute = AutsPerHour / 60;
    private const float AutsPerHour = AutsPerDay / 24;
    private const float AutsPerDay = RealMinutesToDays * AutsPerRealSecond * 60;
    private const float AutsPerSeason = AutsPerDay * DaysPerSeason;
    private const float AutsPerYear = AutsPerSeason * 4;
    ...
    private const float SecondsPerYear = SecondsPerQuarter * 4;
    ...
    public WorldTime(int hour, int minute, int second, int day, int quarter, int year)
    {
        if (hour < 0 || hour > 23)
        {
            DebugUtils.LogError("Hour component should be an int from 0-23. Defaulting to 0.");
        }

        if (minute < 0 || minute > 59)
        {
            DebugUtils.LogError("Minute component should be an int from 0-59. Defaulting to 0.");
        }

        if (second < 0 || second > 59)
        {
            DebugUtils.LogError("Second component should be an int from 0-59. Defaulting to 0.");
        }

        if (day < 1 || day > DaysPerQuarter)
        {
            DebugUtils.LogError(string.Format("Day component should be an int from 1-{0}. Defaulting to 1.", DaysPerQuarter));
        }

        if (quarter < 1 || quarter > 4)
        {
            DebugUtils.LogError("Quarter component should be an int from 1-4. Defauling to 1.");
            quarter = 1;
        }

        if (year < 0)
        {
            DebugUtils.LogError(string.Format("Year component should be an int greater than or equal to {0}. Defaulting to {0}", StartingYear));
    ...
    public string ToString(string format, IFormatProvider provider)
    {
        if (provider == null)
        {
            // This will always fail with our current language file naming scheme, as it doesn't match up with the standard.
            ...
            try
            {
                provider = CultureInfo.GetCultureInfo(LocaleData.CurrentLocale());
            }
    ...
    public string ToString(bool time, bool date)
    {
        ...
        if (time)
        {
            sb.AppendFormat(string.Format(StringUtils.GetLocalizedTextFiltered("comment#time_string"), this));
        }
        ...
        if (date)
        {
            sb.AppendFormat(string.Format(StringUtils.GetLocalizedTextFiltered("comment#date_string"), this));
        }
