...
namespace Animation
{
    ...
    public class SpritenameAnimation
    {
        ...
        public void SetFrame(int frame)
        {
            if (frame >= frameCount)
            {
                DebugUtils.LogErrorChannel("Animation", "SetFrame frame " + frame + " int is larger than array length " + frameCount + ".");
                return;
            }
        ...
        {
            State = xmlNode.Attributes["State"].InnerText;

            delay = float.Parse(xmlNode.SelectSingleNode("Delay").InnerText);
            loops = bool.Parse(xmlNode.SelectSingleNode("Loops").InnerText);
            FlipX = bool.Parse(xmlNode.SelectSingleNode("FlipX").InnerText);
            ValueBased = bool.Parse(xmlNode.SelectSingleNode("ValueBased").InnerText);

            XmlNodeList frameNodes = xmlNode.SelectNodes("Frames/Frame");

            List<string> frameList = new List<string>();
            foreach (XmlNode frameNode in frameNodes)
            {
                frameList.Add(frameNode.InnerText);
            }
