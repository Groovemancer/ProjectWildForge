...
public class Path_RoomGraph
{
    ...
    {
        DebugUtils.LogChannel("Path_RoomGraph", "Entered Path_RoomGraph");

        // Loop through all rooms of the world
        // For each room, create a node
        nodes = new Dictionary<Room, Path_Node<Room>>();
        DebugUtils.LogChannel("Path_RoomGraph", "There are " + world.RoomManager.Count + " Rooms");
        ...
        DebugUtils.LogChannel("Path_RoomGraph", "Created " + nodes.Count + " nodes.");
        ...
        foreach (Room room in nodes.Keys)
        {
            DebugUtils.LogChannel("Path_RoomGraph", "Room " + room.Id + " has edges to:");
            foreach (Path_Edge<Room> edge in nodes[room].edges)
            {
                DebugUtils.LogChannel("Path_RoomGraph", "\tEdge connects to " + edge.node.data.Id);
            }
        }
    }
    ...
    private void GenerateEdgesByRoom(Room room)
    {
        ...
        Dictionary<Tile, Room> neighbours = room.GetNeighbors();
